...
class TestMetricComputer(unittest.TestCase):
    ...
    def test_available_metrics(self):
        ...
        for metric in metric_computer.available_metrics:
            self.assertTrue(
                list(metric.keys())[0] in JETSTREAM_METRIC_DICT.keys()
            )
    ...
    def test_sel(self):
        ...
        self.assertRaises(
            KeyError, lambda: metric_computer.sel(fake=slice(0, 90))
        )
    ...
    def test_isel(self):
        ...
        self.assertRaises(
            ValueError, lambda: metric_computer.isel(fake=slice(0, 90))
        )
    ...
    def test_compute_metric_from_data(self):
        metric_computer = (
            compute_metrics.MetricComputer.with_available_metrics(
                self.data, all_metrics=JETSTREAM_METRIC_DICT
            )
        )
        ...
        bad_metric_computer = compute_metrics.MetricComputer(
            self.data, all_metrics={"1": 1}
        )
        self.assertRaises(
            KeyError,
            lambda: bad_metric_computer.compute_metric_from_data(
                TEST_METRIC_NAME
            ),
        )
...
class TestComputeMetricFunctions(unittest.TestCase):
    ...
    def test_subset_data_with_ignore_coords(self):
        ...
        self.assertRaises(
            AssertionError,
            lambda: tested_func(self.data, test_metric, ignore_coords="wrong"),
        )
        self.assertRaises(
            AssertionError,
            lambda: tested_func(
                self.data, test_metric, ignore_coords=("wrong")
            ),
        )
    ...
    def test_compute_metrics(self):
        ...
        self.assertRaises(
            AssertionError,
            lambda: tested_func("wrong", TEST_METRIC_NAME, {"a": 1}),
        )
        self.assertRaises(
            AssertionError,
            lambda: tested_func(self.data, TEST_METRIC_NAME, [0]),
        )
        self.assertRaises(
            AssertionError,
            lambda: tested_func(self.data["ua"], TEST_METRIC_NAME, {"a": 1}),
        )
        self.assertRaises(
            KeyError, lambda: tested_func(self.data, "wrong", {})
        )
        result = tested_func(
            self.data, TEST_METRIC_NAME, JETSTREAM_METRIC_DICT
        )
        self.assertEqual(float(result["mci"][0][0][0]), -0.015743955969810486)
    ...
    def test_get_available_metric_list(self):
        ...
        self.assertRaises(
            AssertionError, lambda: tested_func("wrong", JETSTREAM_METRIC_DICT)
        )
        result = tested_func(self.data, JETSTREAM_METRIC_DICT)
        self.assertListEqual(
            result,
            [{"FrancisVavrus2015": "usuable"}, {"Ceppi2018": "usuable"}],
        )
    ...
    def test_check_all_chords(self):
        ...
        self.assertRaises(
            AssertionError,
            lambda: tested_func(
                "wrong", JETSTREAM_METRIC_DICT[TEST_METRIC_NAME]
            ),
        )
        metric_usuable, _ = tested_func(
            self.data, JETSTREAM_METRIC_DICT[TEST_METRIC_NAME]
        )
        self.assertTrue(metric_usuable)
        new_data = self.data.sel(plev=1000)
        metric_usuable, _ = tested_func(
            new_data, JETSTREAM_METRIC_DICT["Manney2011"]
        )
        self.assertFalse(metric_usuable)
        metric_usuable, coord_error_message = tested_func(
            new_data,
            JETSTREAM_METRIC_DICT["Manney2011"],
            return_coord_error=True,
        )
        self.assertTrue(
            coord_error_message, "'plev' needs to be between 10000 and 40000."
        )
    ...
    def test_check_all_variables(self):
        ...
        self.assertTrue(
            tested_func(self.data, JETSTREAM_METRIC_DICT[TEST_METRIC_NAME])
        )
