...
def main(data_path, metrics=None, subset=False, subset_kwargs={}, **kwargs):
    ...
    if metrics is None:
        print(
            "Warning: No metric given. Aborting process. Please use '-m' tag to\
             declare metric name and see jetstream_metric_dict.py for a list\
             of all metrics"
        )
        return
    ...
    for metric in metrics:
        ...
        try:
            ...
            if subset:
                print(
                    "Warning: User has chosen to not subset data.\
                       Watch RAM usuage!!"
                )
                subset_kwargs = {"ignore_coords": ["plev"]}
                # TODO: is this necessary? return_coord_error = True
            result = ukesm1_ssp585.compute_metric_from_data(
                metric, subset_kwargs=subset_kwargs
            )
            print("result:", result)
