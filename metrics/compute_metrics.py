...
"""
    Contains the MetricComputer class and functions that run inside that class
    for subsetting and compute metrics from standardised netcdf data
"""
...
class MetricComputer:
    ...
    Q: Why make a data formatter class and not just use functions to handle
       and format the data?
    A: When the data is in an object form, it can ... . Also, allows for
       information hiding.

    (see https://www.datacamp.com/community/tutorials/docstrings-python
     for docstring format)
    """

    def __init__(self, data, all_metrics):
        assert isinstance(data, xr.Dataset), "data needs to be xarray.dataset"
        assert (
            isinstance(all_metrics, dict) and len(all_metrics) > 0
    ...
    def get_available_metrics(self, return_coord_error=False):
        ...
        print(
            "%s metrics available for this dataset:"
    ...
    def sel(self, **kwargs):
        ...
        if hasattr(self, "available_metrics"):
            return MetricComputer.with_available_metrics(
    ...
    def compute_metric_from_data(
        ...
        if not hasattr(self, "available_metrics"):
            try:
                ...
                raise KeyError(
                    "A dictionary of all metrics is required. Error is: %s "
                    % (e)
                )
...
def subset_data(data, metric, ignore_coords=None):
    ...
    if ignore_coords:
        assert isinstance(
            ignore_coords, (list, tuple)
        ), "ignore coords need to be list or set"
    assert isinstance(data, xr.Dataset), "data needs to be xarray.dataset"
    ...
    if ignore_coords:
        ...
        for removed_coord in coords_to_subset.intersection(ignore_coords):
            print(
                "Note:",
                removed_coord,
                "has not been subset for the experiment",
            )
...
def compute_metric(
    ...
    if check_all_coords_available(
        data, all_metrics[metric_name], return_coord_error
    )[0] and check_all_variables_available(data, all_metrics[metric_name]):
        # print('all checks passed')
        pass
    else:
        print(
            "cannot calculate %s metric from data provided" % (metric_name)
...
def get_available_metric_list(
    data, all_metrics=None, return_coord_error=False
):
    """
    ...
    for metric_name in all_metrics:
        ...
        if check_all_variables_available(
            data, metric=all_metrics[metric_name]
        ):
...
def check_all_coords_available(data, metric, return_coord_error=False):
    ...
    try:  # TODO
        ...
        return metric_usable, "%s Metric has no coordinates to subset" % (e)

    # Loop over each coordinate in all metric dictionary
    # and check if the coords exist and can be used for the metric calculation
    for coord in metric["coords"].keys():
        if coord in data.coords:
            coord_vals = metric["coords"][coord]
            coord_available = check_if_coord_vals_meet_reqs(
                data, coord, coord_vals
            )
            # if coord fails check, provide user information why
            if return_coord_error and not coord_available:
                coord_error_message += (
                    " '%s' needs to be between %s and %s."
                    % (
                        str(coord),
                        str(coord_vals[0]),
                        str(coord_vals[1]),
            ...
            # if not exist then break loop as it is required for the metric
            metric_usable = False
            break
