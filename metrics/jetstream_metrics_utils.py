...
"""
    Algorithms and calculations for the metrics used to identify
    or classify jet-stream in the literature.
...
def get_sum_weighted_ws(data, all_plevs_hPa):
    ...
    for plev, (i,plev_hPa) in zip(data['plev'], enumerate(all_plevs_hPa)):
        ...
        sum_weighted_ws += ((data.sel(plev=plev)['ua']** 2 \
                            + data.sel(plev=plev)['va']**2)**(1/2)) * plev_hPa
...
def get_local_jet_maximas_by_day_by_plev(row):
    ...
    """
    row['jet_maxima'] = (('plev', 'lat', 'lon'), np.zeros((row['plev'].size,\
                                                        row['lat'].size, row['lon'].size))) #TODO
    for lon in row['lon']:
        for plev in row['plev']:
            ...
            if len(local_maxima_lat_inds) > 0:
                for lat_ind in local_maxima_lat_inds:
                    row['jet_maxima'].loc[dict(lat=current['lat'].data[lat_ind],\
                                            lon=lon, plev=plev)] = 1.0
    return row
...
def apply_lanczos_filter(data, filter_freq, window_size):
    ...
    window_cons = data['ua'].rolling(time=len(lanczos_weights_arr),\
                                     center=True).construct('window').dot(lanczos_weights_arr)
    return window_cons
...
def assign_lat_ws_to_data(data, max_lat_ws):
    ...
    max_ws = max_lat_ws[:,1]
    data_with_max_lats_ws = data.assign({'max_lats':(('time'),max_lats),\
                                         'max_ws':(('time'),max_ws)})
...
def apply_low_freq_fourier_filter(data, highest_freq_to_keep):
    """
        ...
        highest_freq_to_keep : (int)
            ...
            NOTE: starts at 0, so highest_freq_to_keep=1 will only keep the
                  constant and first expresion
...
def calc_jet_core_per_day(row, ws_core_threshold, ws_boundary_threshold):
    ...
    """
    row['jet_core_id'] = (('plev', 'lat', 'lon'), np.zeros((row['plev'].size, row['lat'].size,\
                                                            row['lon'].size))) #TODO
    for lon in row['lon']:
        current = row.sel(lon=lon)
        core_alg = JetStreamCoreIdentificationAlgorithm(\
                                                    current, ws_core_threshold=ws_core_threshold,\
...
class JetStreamCoreIdentificationAlgorithm:
    """
        "As far as object-oriented design is concerned, the breakdown of an algorithm into steps
         depends on whether the algorithm itself is better seen, from its user's point of view,
         as being executed in one single step, or as multiple steps. If the algorithm is better
         seen as a single step, the object-oriented implementation can choose to hide all of the
        implementation details, leaving only the "inputs, process, outputs" visible to the user."
    """
    def __init__(self, data, ws_core_threshold=40, ws_boundary_threshold=30):
        ...
        self._labelled_data = self._lat_ws_slice.label_slice(self._lat_ws_slice['ws']\
                                                             < ws_core_threshold, 'Core')
        ...
        self._initial_core_ids, self._pot_boundary_ids = self._get_indexes_of_core_and_boundaries()
    ...
    def __repr__(self):
        ...
        if not self.algorithm_has_run:
            print("A total of %d initial Jet-stream cores have been found\
                                 in the wind-speed slice" %\
                    (self._labelled_data['ws'].where(lambda x: x=='Core').count()))
            print("A total of %d potential Jet-stream boundaries have been found\
                                 in the wind-speed slice" %\
                    (self._labelled_data['ws'].where(lambda x: x=='Potential Boundary').count()))
    ...
    def run_algorithm(cls, data, ws_core_threshold=40, ws_boundary_threshold=30):
        ...
        """
        js_algorithm = cls(data, ws_core_threshold=ws_core_threshold,\
                         ws_boundary_threshold=ws_boundary_threshold)
    ...
    def _get_indexes_of_core_and_boundaries(self):
        """
            Will return the indexes in the ws data that ARE jet-stream cores and
            COULD BE jet-stream core boundaries
        """
    ...
    def _make_pot_jetcore_area(self, vals, area, core_found=False):
        ...
        for val in vals:
            ...
            if val in self._initial_core_ids.tolist():
                ...
                if val[1] - self._current_core_lat > 15 and val[1] > self._current_core_lat:
#                     print('THIS IS A NEW CORE')
    ...
    def _attach_cores_to_data(self):
        self._lat_ws_slice.values['core_id'] =  (('plev', 'lat'),\
                             np.zeros((self._lat_ws_slice.values['plev'].size,\
        ...
        for jet_core in self.final_jet_cores:
            for plev, lat in jet_core['index_of_area']:
                self._lat_ws_slice.values['core_id'].loc[\
                                        dict(lat=self._lat_ws_slice.values['lat'].data[lat],\
...
def make_empty_local_wind_maxima_data_var(data):
    """
        ...
        TODO: add asserts
    """
    data['local_wind_maxima'] = (('time','plev', 'lat', 'lon'),\
                                 np.zeros((len(data['time']),len(data['plev']),\
...
def get_local_wind_maxima_by_day(row):
    ...
    for lon in row['lon']:
        ...
        ind_local_wind_maximas = np.array([[arr1, arr2] for arr1, arr2 in\
                                     zip(ind_local_wind_maximas[0], ind_local_wind_maximas[1])])
        for lat_ind, plev_ind in ind_local_wind_maximas:
            row['local_wind_maxima'].loc[dict(lat=current['lat'].data[lat_ind],\
                                             lon=lon, plev=current['plev'].data[plev_ind])] = 1.0
    return row
...
class JetStreamOccurenceAndCentreAlgorithm:
    ...
    def __init__(self, data, occurence_ws_threshold=30):
        ...
        self.plev_ws_slice['jet_ocurrence1_jet_centre2'] =\
                                                 self.plev_ws_slice['jet_ocurrence1_jet_centre2']\
    ...
    def _get_jet_centre_data(self):
        """
            Calculates jet-stream centres based on if one jet-stream occurence grid is surrounded by
            8 cells of jet-stream occurence (default is 30 m/s)
        """
        ## TODO: there's got to be a quicker way
        for centre in self._jet_centres:
            self.output_data['jet_ocurrence1_jet_centre2'].loc[\
                                                dict(lat=centre[0], lon=centre[1])] = 2
    ...
    def _get_all_lats_of_jet_centre_for_search(self):
        ...
        ## Step 2. Check if the latitudes above and below the lats with 3 values are present
        ## i.e. Y component of for 3*3
    ...
    def _get_all_latitudes_available_in_3by3_grid(self):
        for lat in self._lats_with_3:
            if lat-self._lat_resolution in self._lats_with_3 and\
    ...
    def _calculate_jet_centre_points(self):
        ...
        for lat in self._lats_for_search:
            ...
            for coord in coords_to_search:
                if coord[0] == 0 or coord[1] == 0 and 360 - self._lon_resolution\
                         not in coords_to_search[::, 1]:
                    continue
                ## check if coord is jet centre point i.e. 9*9 all above 30
                lat_grid_vals = np.arange(coord[0]-self._lat_resolution,\
                     coord[0]+self._lat_resolution+0.1, self._lat_resolution)
                lon_grid_vals = np.arange(coord[1]-self._lon_resolution,\
                     coord[1]+self._lon_resolution+0.1, self._lon_resolution)
                matrix_vals_to_check = np.array(\
                    np.meshgrid(lat_grid_vals, lon_grid_vals)).T.reshape(-1,2)
...
def meridional_circulation_index(data):
    ...
    When MCI = 0, the wind is purely zonal, and when MCI= 1 (−1),
    the flow is from the South (North).
    ...
    """
    assert 'ua' in data.variables and 'va' in data.variables,\
...
def get_sinousity_of_zonal_mean_zg(row, latitude_circle):
    """
        Works on a grouped data set and will calculate sinuosity of
        zonal mean geopotential (ZG) contour compared to a latitude circle
        Used in Cattiaux et al. 2016
    """
    row['sinousity'] = calculate_great_circle_sinousity(get_one_contour_linestring(\
                                row['zg'], row['zonal_mean_zg_30Nto70N'].data), latitude_circle)
    return row
...
def calculate_great_circle_sinousity(line1, line2):
    """
        ...
        Used in Cattiaux et al. 2016
    """
    return calc_total_great_circle_distance_along_line(line1)\
