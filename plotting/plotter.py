...
class TimePlotter(Plotter, req_coords=('time',)):
    ...
    def time_period_mean(self, starttime, endtime, mean_dims=(...), inplace=False):
        """
            ...
            time_plotter.time_period_mean(starttime=0, endtime=2, mean_dims=('time'), inplace=True)
        """
        ...
        if inplace:
            self.data = self.time_period_sel(starttime, endtime, inplace).mean(mean_dims)
            print('data updated')
            data = self.data
        else:
    ...
    def plot_time_series(self, **kwargs):
        ...
        assert len(self.data.dims) == 1, "Data needs to have exactly 1 dimensions - time"
...
class TimeLatLonPlotter(TimePlotter, req_coords=('lat', 'lon')):
    def map_data(self, data=None, **kwargs):
        ...
        if 'time' in self.data.dims:
            assert self.data['time'].size == 1, "Data has too many dimensions. Data needs to be lat and lon only (2D)" 
