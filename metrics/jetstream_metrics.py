...
def archer_caldeira_2008(data):
    """
        ...
        Similar to Koch et al. 2006 -> "To overcome this problem, we define jet stream properties
        via integrated quantities, which are more numerically stable and less grid-dependent
    ...
    mass_flux_weighted_latitude = jetstream_metrics_utils.calc_mass_flux_weighted_latitude(\
                mon_mean, lat_min=15, lat_max=75)
    data = data.assign({'mass_weighted_average_ws':(('month', 'lat', 'lon'),\
                         mass_weighted_average.data),
                        'mass_flux_weighted_pressure':(('month', 'lat', 'lon'),\
                         mass_flux_weighted_pressure.data) ,\
                        'mass_flux_weighted_latitude':(('month', 'lon'),\
...
def woolings_et_al_2010(data, filter_freq=10, window_size=61):
    """
        Follows an in-text description of 4-steps describing the algorithm mof jet-stream
        identification from Woolings et al. (2010). Will calculate this metric based on
        data (regardless of pressure level of time span etc.).
    ...
    max_lat_ws = np.array(list(map(jetstream_metrics_utils.get_latitude_and_speed_where_max_ws,\
    ...
    fourier_filtered_lats = jetstream_metrics_utils.apply_low_freq_fourier_filter(\
                climatology['max_lats'].values, highest_freq_to_keep=2)
    fourier_filtered_ws = jetstream_metrics_utils.apply_low_freq_fourier_filter(\
    ...
    fourier_filtered_data = jetstream_metrics_utils.assign_filtered_vals_to_data(\
                zonal_mean_lat_ws, fourier_filtered_lats, fourier_filtered_ws, dim=time_dim)
...
def manney_et_al_2011(data, ws_core_threshold=40, ws_boundary_threshold=30):
    ...
    data = data.groupby('time').map(jetstream_metrics_utils.calc_jet_core_per_day,\
...
def francis_vavrus_2015(data):
    """
        Write function description
        Should be lat_min=20, lat_max=80
        MCI
    """
...
def cattiaux_et_al_2016(data):
    ...
    data = data.groupby('time').map(\
            lambda row: jetstream_metrics_utils.get_sinousity_of_zonal_mean_zg(row, circle_50N))
    return data
...
def grise_polvani_2017(data):
    """
        ...
        TODO: work out if relevant as this method also uses poleward edge
        of sub-tropical dry zone and poleward edge of Hadley cell derived from precip. record
    """
    ## Step 1.
    print('Step 1. Calculate zonal-mean')
    zonal_mean = jetstream_metrics_utils.get_zonal_mean(data)
    print('Step 2. Get 3 latitudes and speeds around max zonal wind-speed (lat-1, lat, lat+1)')
    all_max_lats_and_ws = np.array(list(map(\
            jetstream_metrics_utils.get_3_latitudes_and_speed_around_max_ws, zonal_mean['ua'])))
...
def simpson_et_al_2018(data):
    """
        ...
        using a cubic spline interpolation and then isotropically smoothed in
        the spectral domain retaining only scales larger than total wavenumber 42
        according to Sardeshmukh and Hoskins [1984, their Eq. (9) with n0=42 and r=1]. T
    """
    return
...
def bracegirdle_et_al_2019(data):
    ...
    ## Step 3
    print('Step 3. Cubic spline each climatology at latitude resolution of 0.075 degrees')
    seasonal_max_lats, seasonal_max_ws = jetstream_metrics_utils.\
            run_cubic_spline_interpolation_for_each_climatology_to_get_max_lat_and_ws(\
                    seasonal_zonal_mean, resolution=0.075, time_col='season')
    annual_max_lats, annual_max_ws = jetstream_metrics_utils.\
            run_cubic_spline_interpolation_for_each_climatology_to_get_max_lat_and_ws(\
    ...
    data = data.assign({'seasonal_JPOS':(('season'), seasonal_max_lats),\
