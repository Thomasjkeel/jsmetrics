...
ALL_EXPERIMENTS = {
    ...
    "calc-metrics": {
        "description": "will calcualte any number of metrics.\
                        Use -m tag to state which one",
...
def run_experiments(args):
    ...
    if not args["experiment"]:
        print(
            "Error: No experiment declared. Use: '-e experiment_name'\
                 or '-l to list all experiments'"
        )
    elif args["experiment"] not in ALL_EXPERIMENTS.keys():
        print(
            "Error: '%s' is not a known experiment.\
                 Run '-l' for list of experiments"
            % (args["experiment"])
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-e", "--experiment", type=str, help="path to a python script"
    )
    parser.add_argument(
        "-d", "--data_path", type=str, help="path to a netcdf4 data"
    )
    parser.add_argument(
        "-m", "--metrics", nargs="+", help="names of metrics to use"
    )
    parser.add_argument(
        "-s", "--subset", help="should data be subset", action="store_true"
    )
    parser.add_argument(
        "-l", "--ls", help="list all experiments", action="store_true"
    )
