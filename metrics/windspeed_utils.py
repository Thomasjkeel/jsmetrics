...
class WindSpeedSlice():
    ...
    def __init__(self, data, req_variables=('ua', 'va')):
        ## these will check that the correct variables/coords are available in the xarray input
    ...
    def _calc_windspeed(data):
        """
            The reason this exists is that their may be a point in time where constraints
            on this calculation will need to be made i.e. if too much data or too much RAM in use
        """
...
def check_only_required_coords_are_in_data(data, req_coords, to_remove=()):
    ...
    if len(difference) > 0:
        raise ValueError("Unwanted coords in data: %s.\
